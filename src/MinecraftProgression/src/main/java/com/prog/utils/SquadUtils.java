...
public class SquadUtils {
    ...
    public static double getExactPlayerRank(PlayerEntity player) {
        var power = 0D;
        if (player != null) {
    ...
    public static int getPlayerRank(PlayerEntity player) {
        return (int) Math.floor(getExactPlayerRank(player));
    }
}
